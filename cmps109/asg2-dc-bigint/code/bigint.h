...
class bigint {
   ...
   private:
      ...
      quot_rem divide (const bigvalue_t&, const bigvalue_t&) const;
      friend void divide_by_2 (bigvalue_t&);
      bigvalue_t do_bigadd  (const bigvalue_t&, 
                             const bigvalue_t&) const;
      bigvalue_t do_bigsub  (const bigvalue_t&, 
                             const bigvalue_t&) const;
      bigvalue_t do_bigmull (const bigvalue_t&, 
                             const bigvalue_t&) const;
      bool do_bigless(const bigint&, const bigint&) const;
   public:
